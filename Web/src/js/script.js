const CarousselJS = document.querySelector(".CarousselJS");
const CarousselJSCarrousel = CarousselJS.querySelector(".CarousselJS-carrousel");
const carrousel = CarousselJS.querySelector(".carrousel");
...
const createCarrousel = () => {
  ...
  CarousselJS.style.width = tz * 2 + gap * length + "px";
  CarousselJS.style.height = height + "px";
...
const calculateFov = carrouselProps => {
  const perspective = window
    .getComputedStyle(CarousselJSCarrousel)
...
const onResize = () => {
  ...
  const boundingCarrousel = CarousselJSCarrousel.getBoundingClientRect();
...
const initEvents = () => {
  ...
  CarousselJS.addEventListener("mouseleave", () => (isMouseDown = false));
